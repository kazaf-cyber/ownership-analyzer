...
// ============================================================
// /api/resolve — Reconstruct full article URL from Google breadcrumb
// Input:  { domain: "reuters.com", pathHint: ["energy", "pe"], title: "..." }
// Output: { url: "https://www.reuters.com/energy/petrochina-appoints-wu-..." }
// ============================================================
...
if (url.pathname === '/api/resolve' && request.method === 'POST') {
  ...
  if (!domain || !title) {
    return new Response(JSON.stringify({ error: 'Missing domain or title' }), {
      status: 400, headers: { 'Content-Type': 'application/json' },
    });
  }
  
  // Use DuckDuckGo HTML (no API key needed, no JS required)
  // Search for: "title" site:domain
  const query = `"${title.slice(0, 100)}" site:${domain}`;
  const searchUrl = `https://html.duckduckgo.com/html/?q=${encodeURIComponent(query)}`;
  
  try {
    const ddgResp = await fetch(searchUrl, {
      headers: {
    ...
    const match = html.match(/<a[^>]*class="result__a"[^>]*href="([^"]+)"/);
    if (match) {
